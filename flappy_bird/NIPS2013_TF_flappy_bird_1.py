...
def train_model(s, a, y, replay_memory, output, train_step):
...
def main():
    ...
    while time.time() - start_time < 5*60:        
        ...
        while not done and episode_step < 1000:
            ...
            replay_memory.append((stacked_state, action, reward, stacked_next_state, done))
            if len(replay_memory) > size_replay_memory:
                replay_memory.popleft()

            # only train if done observing
            if progress == "Training":
                train_model(s, a, y, replay_memory, output, train_step)
