...
def main():
    ...
    while time.time() - start_time < 5*60:        
        if len(replay_memory) < size_replay_memory:
        ...
        while not done and episode_step < 1000:
            ...
            Qvalue = model.predict(stacked_state)       #input a stack of 4 images, get the prediction
            ...
            if progress == "Training":
                ...
                y_array     = model.predict(states)
                Q_array     = model.predict(next_states)
                
                y_array[range(batch_size), actions] = rewards + discount_factor*np.max(Q_array, axis=1)*np.invert(dones)

                loss += model.train_on_batch(states, y_array)
            ...
            if done or episode_step == 1000:
                ...
                print("Episode :{:>5}".format(episode), "/ Episode step :{:>4}".format(episode_step), "/ Progress :", progress, \
                      "/ Epsilon :{:>2.6f}".format(epsilon), "/ Memory size :{:>5}".format(len(replay_memory)))
                break
